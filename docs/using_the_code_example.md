...
The following example clones the "[mtb-example-psoc-edge-coremark-port](https://github.com/Infineon/mtb-example-psoc-edge-coremark-port)" application with the desired name "CoreMark-Port" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-coremark-port --user-app-name CoreMark-Port --target-dir "C:/mtb_projects"
   ```
