...
portable_init(core_portable *p, int *argc, char *argv[])
{
    ...
    /* Shutdown unused parts of the SOCMEM (Except PARTITION 0) */
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION1, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION2, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION3, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION4, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION5, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION6, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION7, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION8, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);
    Cy_SysPm_SetSOCMemPartActivePwrMode(CY_SYSPM_SOCMEM_SRAM_PARTITION9, CY_SYSPM_SOCMEM_SRAM_ACTIVE_MODE_OFF);

#ifdef BENCHMARK_CM55

    /* Reduce CM33 clock frequency (by changing source and divider) */
    Cy_SysClk_ClkHfSetSource(0, CY_SYSCLK_CLKHF_IN_CLKPATH3);
    Cy_SysClk_ClkHfSetDivider(0, CY_SYSCLK_CLKHF_DIVIDE_BY_16);

#else

    /* Go to deep-sleep if not running the core benchmark */
    for (;;)
    {
        Cy_SysPm_CpuEnterDeepSleep(CY_SYSPM_WAIT_FOR_INTERRUPT);
    }

#endif /* BENCHMARK_CM55 */
...
portable_fini(core_portable *p)
{
    ...
    /* Go to deep sleep after running core benchmark */
    for (;;)
    {
        Cy_SysPm_CpuEnterDeepSleep(CY_SYSPM_WAIT_FOR_INTERRUPT);
    }
